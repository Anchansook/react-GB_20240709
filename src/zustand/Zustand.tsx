...
// - Redux, MobX 등과 같은 타 패키지에 비해 가벼움
//   -> 빌드 시 패키징 속도가 굉장히 빠름 / 번들의 크기가 줄어듦

//# npm i zustand

//# zustand를 이용한 글로벌 상태 선언 방법
// - zustand 패키지에 있는 create 함수로 store를 생성할 수 있음
// - store : 관리할 상태들과 각 상태에 해당하는 상태변경 함수들의 집합
// - create 함수를 호출하면 스토어를 반환하는 훅 함수를 반환

// - create 함수는 매개변수로 set 매개변수를 받는 콜백함수를 요구
// - 매개변수로 받는 set은 상태(store) 변경을 위한 함수
// - create 함수의 매개변수로 전달된 콜백함수는 store 객체를 반환해야 함

// const [address, setAddress] = useState<string>('');

// - TypeScript에서는 zustand의 store 타입을 create 함수의 제너릭으로 전달 해줘야 함
