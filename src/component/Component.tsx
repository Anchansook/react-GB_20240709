...
// 컴포넌트 선언 방법
// 1. class component (이건 이런게 있다~ 정리만!)
// - class로 컴포넌트 작성
// - 반드시 첫글자는 대문자
...
// - 함수로 컴포넌트를 작성
// - '반드시 첫글자를 대문자'로 작성해야 컴포넌트 함수로 인식
// - 함수의 반환값으로 렌더링하고자 하는 요소를 반환
export function FunctonComponent () {
    return (
        <h1>함수형 컴포넌트</h1>
    )
}; 
...
export default function Component () {
    ...
    //& - 여러가지 JS, TS *연산자*를 사용할 수 있음!

    // - 반드시 하나의 요소만 반환 가능 (0개 X, 2개 이상 X)
    // - 다수의 요소를 반환하고 싶다면 최상단에 부모 요소로 묶어줘야 함
    // - 부모 요소로 묶을 때 <></>(Fragment)를 이용하여 의미없는 태그를 생성할 수 있음
    ...
    // 1. 반드시 하나의 요소를 반환
    // 2. 모든 태그를 닫아야 함
    // 3. 속성(태그 내 속성)들을 JSX로 지정된 이름을 사용해야 함 (lowerCamelCase)
    // 4. 주석을 사용할 땐 {/* */}를 사용
