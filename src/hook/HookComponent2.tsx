...
export default function HookComponent2() {
    ...
    // - const 변수 = useRef<요소의 타입>(초기값);

    // DOM 요소를 참조하기 위해 useRef 사용할 경우 
    // 참조할 요소의 ref 속성에 참조변수를 지정해야 함
    ...
    const onButtonClickHandler = () => {
        ...
        // : 특정 객체에서 속성 및 메서드를 호출하려할 때 
        //   변수의 값이 null 혹은 undefined가 아닌 상태에서만 작업을 수행하도록 하려면 
        //   '변수?.속성' 혹은 '변수?.메서드()'로 지정
    ...
    const onImageButtonClickHandler = () => {
        ...
        if (!current) return;
        current.click(); 
        // ▲ imageInputRef가 등록된 input과 같은 작업을 클릭 시에 수행하라!
    };
