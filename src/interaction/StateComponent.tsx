...
//# 상태 (state) :
// - 각각의 컴포넌트가 독립적으로 가지고 있는 데이터 저장 공간(메모리 공간)
...
export default function StateComponent() {
    ...
    const onIncrease = () => {
        ...
        // 상태를 변경하는 작업이 한번에 여러번 실행된다면 임시변수 활용하여 사용할 수 있음
        // let tmp = 0;
        // tmp = count; // or let tmp = count;
        // tmp += 1;
        // tmp += 2;
        // tmp += 3;
        // setCount(tmp);
        ...
        // 아래 코드는 setTotal에서 기존의 count값(바뀌기 전)을 사용하기 때문에
        // 동작이 한단계 전 단계로 동작함
        // setCount(count + 1);
    ...
    return (
        <div>
            <h1>{comm}</h1>
            <h1>{comment}</h1>
            {/* 만약 input으로 상태를 변경한다면 'value'로 그 '상태'를 지정해야 불일치가 발생하지 않음 */}
            <input value={comment} onChange={onChangeHandler} type="text" />
            <h1>count : {count}</h1>
            <h1>total : {total}</h1>
